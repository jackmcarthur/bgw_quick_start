...
* The dielectric function in metals may differ significantly from those predicted in plasmon-pole approximations. Full-frequency calculations are recommended; they are rigorously correct in these cases. Full frequency calculations also give access to the plasmon dispersion of the system, as the plasmon appears as a large peak in $\varepsilon^{-1}_{G=G'=0}(q,\omega)$ (see `epsinvomega` tool documentation).
