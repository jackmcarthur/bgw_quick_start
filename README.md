...
* As in 2D and 1D materials, `surface.x` is useful for obtaining the recommended supercell size.
...
* BSE calculations on molecules may require unusually many conduction bands (50+) to converge exciton binding energies. 
...
* The dielectric function in metals may differ significantly from those predicted in plasmon-pole approximations. Full-frequency calculations are recommended; they are rigorously correct in these cases. Full frequency calculations also give access to the plasmon dispersion of the system, as plasmons appears as a large peak in $\varepsilon^{-1}_{G=G'=0}(q,\omega)$ (see `epsinvomega` tool documentation).
...
## Epsilon
* There are three possible values for the `frequency_dependence` flag.
  * `0` (default) is to calculate only the zero-frequency dielectric matrix. This is used in the Hybertsen-Louie Generalized Plasmon Pole Model (HL-GPP), which offers ~0.1-0.2 eV accuracy of the QP energies near the Fermi energy at low cost.
  * `3` is to compute the two frequencies used by the Godby-Needs Plasmon Pole Model, which is known to perform better in certain systems with localized electrons, see [[1]](https://journals.aps.org/prb/pdf/10.1103/PhysRevB.88.125205) and other literature.
  * `2` is for full-frequency calculations, which calculate the dielectric matrix across the whole frequency range, increasing accuracy and giving access to spectral functions and lifetimes. It is also more accurate further from the Fermi energy and in systems with localized d/f electrons, though it has a much greater cost. However, the Static Subspace Approximation (SSA) (see `Epsilon` documentation) can allow for full-frequency calculations with only a small overhead compared to the HL-GPP.
...
* All `Absorption` calculations require either the flag `use_momentum` or `use_velocity`, which controls whether you need only one fine k-grid `WFN_fi` or an additional shifted fine k-grid `WFNq_fi`.
* There are several diagonalization methods implemented in `Absorption`.
  * The flags `haydock`, `diagonalization`+`lanczos`, and `diagonalization`+`diagonalization_primme` all enable iterative calculations, where the lowest $N$ eigenvalues are determined. By itself, the `diagonalization` flag will use ScaLAPACK (or ELPA if compiled) to diagonalize the entire spectrum.
  * If you are only interested in the lowest part of the BSE spectrum (less than 5% of the rank of the Hamiltonian, which is $N_k N_c N_v$), the iterative methods will be faster than the full eigensolver. This is useful if you are chiefly interested in the sub-gap exciton states/the absorption spectrum only near and below the band gap. If you want to converge the higher-energy parts of your absorption spectrum, well above the gap, full diagonalization may be better. Note also that the absorption spectrum will be converged up to, at maximum, the energy difference of your highest included conduction band and lowest included valence band.
  * The Haydock method returns only an absorption spectrum and cannot return the eigenvectors (BSE envelope functions $A_{cvk}^S$); the other methods allow `write_eigenvectors #` to determine how many eigenvectors are written to `eigenvectors.h5`.
* **Dipole matrix elements**:
  * Recall that the exciton energies/BSE eigenvalues and exciton envelope functions/BSE eigenvectors do not depend on the method used to calculate dipole matrix elements; they depend only on the quasiparticle energies `eqp_co.dat` and the kernel matrix elements, which depend only on the wavefunctions and dielectric matrix. These are the only ingredients in the BSE Hamiltonian `\hat{H}_{BSE}` which is diagonalized. The dipole matrix elements affect *only the heights of peaks in the absorption spectrum*, they do not affect peak positions. 
  * `use_momentum` and `use_velocity` control how the dipole matrix elements are calculated in the code. These two flags exist because the oscillator strength of an exciton state $|S\rangle$ is proportional to the dipole matrix element $|\langle S|\hat{e}\bullet\mathbf{\hat{v}}|0 \rangle|^2$ for light polarization along $\hat{e}$, where $\vec{v}$ is the velocity operator. The true definition of the $\vec{v}$ operator is $-i[\hat{r},\hat{H}]$; this definition is used with `use_velocity`, and its evaluation requires the `WFNq_fi` file with a k-grid shifted along the direction of $\hat{e}$ relative to `WFN_fi`.
  * The above definition includes the non-local part of the pseudopotentials used, but when the Hamiltonian is fully local, $\hat{{v}}$ reduces to just $\hat{p}/m$. This simpler operator does not require `WFNq_fi`, just specify the polarization vector as `polarization 0.707 0.707 0` or whatever desired vector in *crystal* x/y/z coordinates. This simply drops the contribution from the non-local parts of the pseudopotentials, and will introduce error into the oscillator strengths if those parts are large.
  * The polarization direction is that of linear polarization in crystal coordinates. See `BSE/MagneticSpin/circularly_polarized_light_absorption.ipynb` as a guide to obtain circularly polarized absorption. In short, you can perform two `Absorption` calculations along cartesian x and y, then take the linear combinations $e_x \pm i e_y$ of the dipole elements in the third/fourth columns of `eigenvalues.dat`, and plot those with your desired broadening to get the circularly polarized absorption spectra.
...
* **Number of unoccupied bands in the dielectric matrix summation/Coulomb-hole summation**: These scale as the number of atoms in your unit cell, making it hard to give ballpark values, but it is generally larger when atoms with semicore electrons or d/f electrons are present. For silicon (2 atoms in unit cell), 100 bands are necessary for ~50 meV accuracy. For simple transition metal oxides, 1000+ bands are needed with 3000+ in the most pathological cases. `Epsilon` and `Sigma` can be sped up enormously if you compress the unoccupied bands in the `WFN` file with Stochastic Pseudobands, included with BerkeleyGW.
* It is rather common to use the same number of bands for both the dielectric matrix summation and the Coulomb-hole summation. However, of the two, the dielectric matrix summation generally converges slightly faster with the number of bands. Since `Epsilon` is the bottleneck of the two for large systems, one can converge the two cutoffs separately, generally expecting that the `Epsilon` cutoff will be slightly fewer.
