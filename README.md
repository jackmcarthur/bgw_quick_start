...
** Dipole matrix elements:
  * Recall that the exciton energies/BSE eigenvalues and exciton envelope functions/BSE eigenvectors are not dependent on the method used to calculate dipole matrix elements; they depend only on the quasiparticle energies `eqp_co.dat` and the kernel matrix elements, which depend only on the wavefunctions and dielectric matrix. These are the only ingredients in the BSE Hamiltonian `\hat{H}_BSE` which is diagonalized. The dipole matrix elements affect *only the heights of peaks in the absorption spectrum*, they do not affect peak positions. 
