...
* There are three possible values for the `frequency_dependence` flag.
  * The default `0` is to calculate only the zero-frequency dielectric matrix. This is used in the Hybertsen-Louie Generalized Plasmon Pole Model (HL-GPP), which offers ~0.1-0.2 eV accuracy of the QP energies near the Fermi energy at low cost.
  * Another option, `3`, is to compute the two frequencies used by the Godby-Needs Plasmon Pole Model, which is known to perform better in certain systems with localized electrons, see [[1]](https://journals.aps.org/prb/pdf/10.1103/PhysRevB.88.125205) and other literature.
